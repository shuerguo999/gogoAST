...
const api = {
    // 通过选择器获取，返回ast片段
    getAstsBySelector(ast, selector, strictSequence = true, deep) { 
        //strictSequence作用：
    ...
    replaceSelBySel(ast, selector, replacer, strictSequence = true) {
        // 用于结构不一致的，整体替换
        const { nodePathList, matchWildCardList } = api.getAstsBySelector(ast, selector, strictSequence);
    ...
    modifySelBySel(ast, selector, replacer) {
        ...
        const { nodePathList, matchWildCardList } = find.call(ast, selectorAst.nodeType, selectorAst.structure, 'nn');
    ...
    removeAst(ast, selector) {
        ...
        const { nodePathList } = find.call(ast, selectorAst.nodeType, selectorAst.structure, 'nn');
        // console.log(nodePathList)
        nodePathList.forEach(path => {
