...
module.exports = async function(GG, { pluginDir, codeList, outputFile = true, deleteComment = false }) {
    ...
    codeList.forEach((filePath, index) => {
        ...
        ruleList.forEach(rule => {
            if (!transformMap[rule.go.match]) {
                transformMap[rule.go.match] = [];
            }
            transformMap[rule.go.match].push(rule.go)
        })
        traverse(ast, transformMap, isPath ? filePath : '', deleteComment);
        transformMap.document.forEach(docTranc => {
            docTranc.handle(ast, { filePath })
        })
        let tmpl = generator(ast);

        if (isPath && outputFile) {
            const suffix = filePath.match(/\.(htm|html|axml|tpl)/)[0];
